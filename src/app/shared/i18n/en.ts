import enSlideSet1Translations from '@slides/slide-set1/i18n/en';

const en = {
  ui: {
    menu: '\\Menu',
    settings: '\\Settings',
    siteTitle: '\\Teodora Chiosa Slides',
    skipToPresent: '\\Skip to "Present"',
    skipToNavigation: '\\Skip to "Menu"',
    skipToSlides: '\\Skip to main content',
    noteTitle: '\\Note',
    noteText: '\\Changes apply instantly',
    contentView: '\\Slides view',
    slideView: '\\Fixed aspect ratio',
    slideViewDescription: '\\Maintain an aspect ratio of 16:9 (zoom not supported).',
    webView: '\\Web',
    webViewDescription: '\\Allow web page behavior (content can be zoomed).',
    maxWidth: '\\Maximum width',
    reset: '\\Reset',
    decreaseMaxWidth: '\\Decrease maximum width',
    increaseMaxWidth: '\\Increase maximum width',
    theme: '\\Theme',
    light: '\\Light',
    dark: '\\Dark',
    system: '\\System',
    language: '\\Language',
    present: '\\Present',
    notFoundTitle: '\\Page not found',
    notFoundSubtitle: '\\Try one of the pages from the menu instead',
    footerText: '\\Custom footer content goes here...',
    iconsCredit: '\\Icons by',
    external: '\\external'
  },
  sets: {
    set1: enSlideSet1Translations,
  },
};

export default en;
